...
import { ArrowLeft, Loader2, AlertTriangle, Zap, Code, CheckCircle, Server, Folder } from 'lucide-react';

const API_BASE_URL = process.env.REACT_APP_API_URL || 'https://althaf-portfolio.onrender.com';

// ✨ NEW: Helper function to check if a line is a code example
const isCodeLine = (line) => {
  const trimmedLine = line.trim();
  // Keywords that often start code lines in your examples
  const codeKeywords = ['FROM', 'WORKDIR', 'COPY', 'RUN', 'ENV', 'CMD', 'pipeline', 'agent', 'stages', 'stage', 'steps', 'sh', 'docker', 'kubectl', 'helm'];
  // Check for common code patterns (e.g., # comments, significant indentation)
  return trimmedLine.startsWith('#') || codeKeywords.some(keyword => trimmedLine.toLowerCase().startsWith(keyword)) || line.startsWith('    ');
};
    ...
    <div className="min-h-screen bg-black py-16 px-4 sm:px-6 lg:px-8">
      ...
      {project && (
        <Card className="max-w-4xl mx-auto p-8 bg-black/80 border border-gray-700/30">
          ...
          {/* ✨ MODIFIED: Summary with Zap icons */}
          ...
          {/* ✨ MODIFIED: Key Outcomes with CheckCircle icons */}
          <section className="mb-8">
          ...
          {/* ✨ MODIFIED: Details with conditional icons */}
          <section>
              ...
              {project.details.split('\n').map((line, idx) => (
                <div key={idx} className="flex items-start space-x-3 text-gray-300">
                  {isCodeLine(line) || line.trim() === '' ? (
                    <div className="w-4 flex-shrink-0"></div> // Empty spacer for code lines
                  ) : (
