...
const ContactSection = ({ personalInfo }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // ✨ CORRECTED: The keys now match the backend's Pydantic model
    const payload = {
      name: formData.name,
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          <div className={`space-y-8 ${isVisible ? 'fade-in-left stagger-2' : ''}`}>
            ...
            <div className="space-y-4">
              <Card className={`p-4 bg-black/80 border border-gray-700/30 transition-all ${isVisible ? 'scale-in stagger-3' : ''}`}>
              ...
              <Card className={`p-4 bg-black/80 border border-gray-700/30 transition-all ${isVisible ? 'scale-in stagger-4' : ''}`}>
                 <div className="flex items-center space-x-4">
          ...
          <Card className={`p-8 bg-black/80 border border-gray-700/30 ${isVisible ? 'fade-in-right stagger-8' : ''}`}>
