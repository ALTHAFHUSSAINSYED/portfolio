...
```mermaid
graph TB
    User[User Browser] --> Amplify[AWS Amplify<br/>Frontend CDN]
    User --> EC2[EC2 Instance<br/>Backend API]
    
    Amplify --> S3_Blogs[S3 Bucket<br/>Blog Storage]
    
    EC2 --> Docker[Docker Container<br/>FastAPI App]
    ...
    Actions --> EC2_Runner[Self-Hosted Runner<br/>on EC2]
    EC2_Runner --> Docker
    Actions --> Amplify
```
...
graph TB
    ...
    Actions --> Runner[Self-Hosted Runner<br/>on EC2]
    
    Runner --> Pull[git pull origin main]
    Pull --> Kill[Kill process on port 8000]
    Kill --> Clean[Docker system prune]
    Clean --> Build[docker build --no-cache]
    Build --> Run[docker run -d]
    Run --> Health[Health check]
    
    Health -->|Success| Done[✅ Deployment Complete]
    Health -->|Fail| Logs[Show logs & exit 1]
```

**Workflow File (`backend-deploy.yml`):**
```yaml
...
graph TB
    Push[git push origin main] --> GitHub[GitHub Repository]
    GitHub --> Amplify[AWS Amplify<br/>Auto-detects changes]
    
    Amplify --> Install[yarn install]
...
- ✅ HTTPS (via Amplify for frontend)
...
5. ✅ Static frontend (Amplify free tier)
